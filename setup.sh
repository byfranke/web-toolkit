...
if [ -f "requirements.txt" ]; then
  ...
  pip3 install --user cryptography paramiko python-whois requests colorama tabulate rich prompt_toolkit
fi

echo "[*] Checking additional system dependencies..."
for tool in nmap sqlmap wget curl gobuster nuclei subfinder whatweb; do
    ...
    if command -v apt-get &> /dev/null; then
      case "$tool" in
        "nuclei")
          ...
          wget -O nuclei.zip https://github.com/projectdiscovery/nuclei/releases/latest/download/nuclei_2.9.4_linux_amd64.zip 2>/dev/null || echo "Failed to download nuclei"
          [ -f nuclei.zip ] && unzip -o nuclei.zip && sudo mv nuclei /usr/local/bin/ && rm nuclei.zip
          ;;
        "subfinder")
          wget -O subfinder.zip https://github.com/projectdiscovery/subfinder/releases/latest/download/subfinder_2.6.3_linux_amd64.zip 2>/dev/null || echo "Failed to download subfinder"
          [ -f subfinder.zip ] && unzip -o subfinder.zip && sudo mv subfinder /usr/local/bin/ && rm subfinder.zip
          ;;
        *)
      ...
      echo "[!] No package manager found. Please install $tool manually."
    fi
  else
    echo "[+] $tool found."
  fi
...
if command -v apt-get &> /dev/null; then
  ...
  echo "[!] Installing seclists manually..."
